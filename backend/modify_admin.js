const fs = require('fs');
const path = 'd:/Digiline Cloud/Interior Design Website/frontend/src/pages/AdminDashboard.jsx';
let content = fs.readFileSync(path, 'utf8');

// 1. Add states for home settings
if (!content.includes('const [homeSubTab, setHomeSubTab]')) {
    content = content.replace(
        'const [heroSlides, setHeroSlides] = useState([]);',
        `const [heroSlides, setHeroSlides] = useState([]);
    const [homeSubTab, setHomeSubTab] = useState('slides');
    const [homeSettings, setHomeSettings] = useState({
        about: { kicker: '', title: '', desc1: '', desc2: '', buttonText: '', buttonLink: '', image: '' },
        whyChoose: { kicker: '', title: '', features: [
...
// 2. Add fetch function and call it
if (!content.includes('fetchHomeSettings()')) {
    content = content.replace(
        'fetchHeroSlides(),',
        `fetchHeroSlides(),\n                fetchHomeSettings(),`
    );
    ...
    content = content.replace(
        ...
        try {
            ...
            if (res.ok && data.settings) {
                // Ensure features default array length if missing
...
// 3. Add Save Function
if (!content.includes('handleSaveHomeSettings')) {
    content = content.replace(
        'const fetchHeroSlides = async () => {',
...
// 5. Build the UI
const heroHeaderOld = `                            <div className="panel-header">
                                <div>
...
const heroHeaderNew = `                            <div className="panel-header">
                                <div>
...
if (content.includes(heroHeaderOld)) {
    content = content.replace(heroHeaderOld, heroHeaderNew);
    const heroHeaderFooterOld = \`})}
                                </div>
                            )}
                        </div>
                    )}\`;
    const heroHeaderFooterNew = \`})}
                                </div>
                            )}
                                </>
                            )}
    ...
    content = content.replace(heroHeaderFooterOld, heroHeaderFooterNew);
}
